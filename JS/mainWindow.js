      ...
      codeblock += 
        `
          <div class="row">
            <div class="col">
              <div class="form-group">
                <label for="arrival-`+i+`">Arrival Time P`+i+`</label>
...
function submitform(e) {
  ...
  for (let j = 1; j <= no_of_Processes; j++) {
    ...
    processes[j-1] = { p: "p" + j, a: Number(arrival), b: Number(burst), priority: priority != null ? Number(priority) : priority};
  }
  ...
  if(type_of_scheduler == 'FCFS'){
    ...
    let q_time = document.getElementById("q-time").value;
...
function sort_nonpreemptive(process, type){
  ...
  while(total_burst_time > 0){
    ...
    if(arrived_elements.length > 0){
      ...
      /** free time */
      let starttime = ready_queue[ready_queue.length -1] ? ready_queue[ready_queue.length -1]['b']: 0;
